CREATE OR REPLACE NONEDITIONABLE PACKAGE passport_dev IS
    ...
    PROCEDURE prepare (tin$ NUMBER, dt1$ VARCHAR2, dt2$ VARCHAR2, work$ VARCHAR2, guid_user$ VARCHAR2, guid$ VARCHAR2 DEFAULT '');

    PROCEDURE create_job (tin$ NUMBER, dt1$ VARCHAR2, dt2$ VARCHAR2, work$ VARCHAR2, guid_user$ VARCHAR2, guid$ VARCHAR2 DEFAULT NULL);

END;
/

CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY passport_dev IS
    ...
    FUNCTION get_c_distr_by_tin (tin$ NUMBER) RETURN NUMBER IS FResult NUMBER;
    BEGIN
        BEGIN
    ...
    IS
        PRAGMA AUTONOMOUS_TRANSACTION;
    BEGIN
        INSERT INTO PIKALKA.pass_work
        SELECT GUID#, id, NULL FROM PIKALKA.d_pass_info
        WHERE INSTR(',' || work$ || ',', ',' || id || ',') > 0;
        COMMIT;
    END;
    ...
    BEGIN
        ...
        IF count# = 0 THEN
            job# := pref# || guid_user$ || chr(10) || 'BEGIN PIKALKA.' || package# || '.prepare('
                || tin$ || ', '
                || q(dt1$) || ', '
                || q(dt2$) || ', '
                || q(work$) || ', '
